...
export default function ParceriasPipeline({ allProspects }) {
  ...
  const counts = STAGES.map(s =>
    all.filter(p => p.status === s.key || (p.statusHistory || []).includes(s.key)).length
  )
  const maxC   = Math.max(...counts, 1)
  const taxa   = counts[0] > 0 ? Math.round((counts[4] / counts[0]) * 100) : 0
  const perda  = counts[0] > 0 ? Math.round((counts[5] / counts[0]) * 100) : 0
  const pot    = all.reduce((s,p) => s+(p.value||0), 0)
  const rec    = all.filter(p => p.status === 'recebido').reduce((s,p) => s+(p.finalValue||0), 0)
  const tkt    = counts[4] > 0 ? Math.round(all.filter(p=>p.phase>1).reduce((s,p)=>s+(p.finalValue||0),0) / counts[4]) : 0
      ...
      <div className="grid gap-8" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          {STAGES.map((s, i) => {
            ...
            return (
              <div key={s.key} className="mb-1.5 flex justify-start">
                <div className="h-12 rounded-lg flex items-center px-5 transition-all" style={{ background: s.color, border: `1px solid ${s.text}30`, width: `${w}%`, minWidth: 160 }}>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl p-4 text-sm leading-relaxed" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: 'var(--text3)' }}>
            ...
            Cada barra mostra quantas prospecções já passaram por aquele estágio — inclusive as que avançaram.<br /><br />
